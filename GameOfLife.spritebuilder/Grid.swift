...
//  GameOfLife
//
//  Created by Iavor Dekov on 6/25/15.
...
class Grid: CCSprite {
    ...
    func creatureForTouchPosition(touchPosition: CGPoint) -> Creature {
        // clampf is used to restrict the row and column numbers so that
        // an array out of bounds error does not occur when a person clicks on the right or bottom edges
        // of the grid
        var row = Int(clampf(Float(touchPosition.y / cellHeight), Float(0), Float(7)))
        var column = Int(clampf(Float(touchPosition.x / cellWidth), Float(0), Float(9)))
